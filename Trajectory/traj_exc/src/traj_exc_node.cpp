...
ros::Publisher control_pub_, show_pub_, trigger_pub_, command_real_pub_, quadr_cmd_pub;
ros::Subscriber traj_sub_, sensor_ready_sub_;
...
bool ready_, sen_ready_;
Eigen::Vector3d robot_pos_, takeoff_pos_, takeoff_init_;
double robot_yaw_;
...
void Takeoff(const std_msgs::EmptyPtr &msg){
    ...
    real_pt.type_mask = 0;
    real_pt.position.x = takeoff_init_(0);
    real_pt.position.y = takeoff_init_(1);
    real_pt.position.z = takeoff_init_(2);
    real_pt.yaw = 0;
    real_pt.yaw_rate = 0;
    ...
    // trajectory_point.position_W.x() = takeoff_init_(0);
    // trajectory_point.position_W.y() = takeoff_init_(1);
    // trajectory_point.position_W.z() = takeoff_init_(2);
    // trajectory_point.orientation_W_B.w() = 1.0;

    quadrotor_msgs::PositionCommand cmd;
    cmd.header.stamp = ros::Time::now();
    cmd.trajectory_id = 0;
    cmd.position.x = takeoff_init_(0);
    cmd.position.y = takeoff_init_(1);
    cmd.position.z = takeoff_init_(2);
    cmd.velocity.x = 0.0;
    cmd.velocity.y = 0.0;
    cmd.velocity.z = 0.0;
    ...
    for(int i = 0; i < 30; i++){
        ...
        real_pt.position.z = (takeoff_pos_(2) + 0.8 - takeoff_init_(2)) * i / 30 + takeoff_init_(2);
        // trajectory_point.position_W.z() = (takeoff_pos_(2) + 0.8 - takeoff_init_(2)) * i / 30 + takeoff_init_(2);
        cmd.position.z = (takeoff_pos_(2) + 0.8 - takeoff_init_(2)) * i / 30 + takeoff_init_(2);
        // cout<<"ti:"<<takeoff_init_(2)<<"  ss:"<<trajectory_point.position_W.transpose()<<endl;
        ...
        quadr_cmd_pub.publish(cmd);
    }

    for(int i = 0; i < 30; i++){
        ros::Duration(0.033).sleep();
        // trajectory_point.position_W.x() += 1.5 / 30;
        // trajectory_point.position_W.z() -= 0.8 / 30;
        // trajectory_point.orientation_W_B.z() = sin(M_PI * i / 29 );
        // trajectory_point.orientation_W_B.w() = cos(M_PI * i / 29 );
        real_pt.position.x += 1.8 / 30;
        real_pt.position.z -= 0.8 / 30;
        real_pt.yaw = 2 * M_PI * i / 29;
        cmd.position.x += 1.2 / 30;
        cmd.position.z -= 0.0 / 30;
        cmd.yaw = 2 * M_PI * i / 29;
        cmd.velocity.x = 1.2 / 30 / 0.033;
        cmd.velocity.z -= 0.0 / 30 / 0.033;
        // mav_msgs::msgMultiDofJointTrajectoryPointFromEigen(trajectory_point, &trajectory_point_msg);
        // samples_array.points.clear();
        // samples_array.points.push_back(trajectory_point_msg);
        // control_pub_.publish(samples_array);
...
void RunCallback(const ros::TimerEvent &e){
    ...
    real_pt.header.stamp = ros::Time::now();
    command_real_pub_.publish(real_pt);
...
void SensorReadyCallback(const std_msgs::EmptyConstPtr& msg){
    if(sen_ready_) return;
    ROS_WARN("sensor ready");
    sen_ready_ = true;
}
...
int main(int argc, char** argv){
    ...
    quadr_cmd_pub = nh.advertise<quadrotor_msgs::PositionCommand>("/planning/pos_cmd", 50);
    ...
    sensor_ready_sub_ = nh.subscribe("/sensor/ready", 1, &SensorReadyCallback);
    ...
    nh_private.param(ns + "/Exp/takeoff_init_x", takeoff_init_(0), 0.0);
    nh_private.param(ns + "/Exp/takeoff_init_y", takeoff_init_(1), 0.0);
    nh_private.param(ns + "/Exp/takeoff_init_z", takeoff_init_(2), 1.0);
