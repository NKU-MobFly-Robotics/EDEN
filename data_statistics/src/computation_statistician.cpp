...
void ComputationStatistician::init(ros::NodeHandle &nh, ros::NodeHandle &nh_private){
    ...
    tm* t=localtime(&now);
    string command;
    path_ = dir_+/*"/"+to_string(id)*/+"/"+to_string(t->tm_year+1900)+"_"+to_string(t->tm_mon+1)+"_"+to_string(t->tm_mday)
...
void ComputationStatistician::SetVolume(double v, double t, int id){
    if(id < 0 || id >= vdata_size_){
        printf("\033[0;31m ERROR dataid! The dataid should be less than %d\033[0m\n", vdata_size_);
        return;
    }
