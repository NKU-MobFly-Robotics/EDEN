...
public: 
     ...
     */
    void UpdateFrontier(const vector<Eigen::Vector3f> &pts);
    ...
    inline int Pos2Idx(const Eigen::Vector3f &pos);
    ...
    inline bool InsideExpMap(const Eigen::Vector3f &pos);
...
private:
    ...
    void Debug(list<Eigen::Vector3d> &pts_out, list<Eigen::Vector3d> &pts_dead, list<Eigen::Vector3d> &pts_infea);
    ...
    vector<double> vp_sample_range_, VpSampleZ_;
...
inline bool EroiGrid::InsideMap(const Eigen::Vector3f &pos){
    if(pos(0) < low_bd_(0) || pos(1) < low_bd_(1) || pos(2) < low_bd_(2) ||
        pos(0) >  up_bd_(0) || pos(1) > up_bd_(1) || pos(2) > up_bd_(2) )
        return false;
    return true;
...
inline bool EroiGrid::InsideExpMap(const Eigen::Vector3f &pos){
    if(pos(0) < exp_lowbd_(0) || pos(1) < exp_lowbd_(1) || pos(2) < exp_lowbd_(2) ||
        pos(0) >  exp_upbd_(0) || pos(1) > exp_upbd_(1) || pos(2) > exp_upbd_(2) )
        return false;
    return true;
...
inline int EroiGrid::Pos2Idx(const Eigen::Vector3f &pos){
    if(InsideMap(pos)){
        Eigen::Vector3d dpos = pos.cast<double>() - origin_;
        Eigen::Vector3i posid;
        posid.x() = floor(dpos.x() / node_scale_(0));
        posid.y() = floor(dpos.y() / node_scale_(1));
        posid.z() = floor(dpos.z() / node_scale_(2));
