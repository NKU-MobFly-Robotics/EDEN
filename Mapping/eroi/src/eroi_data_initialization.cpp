...
void EroiGrid::AlignInit(const ros::NodeHandle &nh, 
    ...
    nh_private_.param(ns + "/Frontier/VpSampleVerRes", 
        VpSampleVerRes_, 1.0);
    nh_private_.param(ns + "/Frontier/VpSampleZ", 
        VpSampleZ_, {0.0});
    ...
    for(int i = 0; i < vp_sample_range_.size(); i++){
        for(auto z : VpSampleZ_){
            ...
            for(double ang = -M_PI; ang < M_PI; ang += dir + 1e-3){
                ...
                bool add = true;
                for(auto vp : vps_){
                    if((p.head(3) - vp.head(3)).norm() < LRM_->node_scale_.norm()){
                        add = false;
                        break;
                    }
                }
    ...
    if(samp_total_num_ >= 256) {
        cout<<"samp_total_num_:"<<samp_total_num_<<endl;
        StopageDebug("AlignInit sample too many");
    }
            ...
            for(v_it(0) = 0; v_it(0) < node_num_(0); v_it(0)++){
                ...
                for(int dim = 0; dim < 3; dim++){
                    ...
                    if(EN.up_(dim) < exp_lowbd_(dim)){
                        ...
                        double d, u;
                        d = max(EN.down_(dim) - 1e-3, exp_lowbd_(dim) - 1e-3);
                        u = min(EN.up_(dim) + 1e-3, exp_upbd_(dim) + 1e-3);
                ...
                // if(EN.up_(0) > 4.844 && EN.down_(0) < 4.844 && EN.up_(1) > -3.2 && EN.down_(1) < -3.2
                //          && EN.up_(2) > 3.5 && EN.down_(2) < 3.5){
                //     cout<<"key idx:"<<EROI_.size()<<endl;
                // }
                ...
                if(dead_eroi){
                    ...
                    EN.unknown_num_ = vox_num(0) * vox_num(1) * vox_num(2);
                    EN.uorigin_ = vox_num(0) * vox_num(1) * vox_num(2);
                    // if(319872 == EROI_.size()){
                    //     cout<<"EN.unknown_num_:"<<EN.unknown_num_<<endl;
                    //     cout<<"EN.up_:"<<EN.up_.transpose()<<endl;
                    //     cout<<"EN.down_:"<<EN.down_.transpose()<<endl;
                    //     cout<<"exp_upbd_:"<<exp_upbd_.transpose()<<endl;
                    //     cout<<"exp_lowbd_:"<<exp_lowbd_.transpose()<<endl;
                    //     for(int dim = 0; dim < 3; dim++){
                    //         double d, u;
...
void EroiGrid::InitGainRays(double geh){
    ...
    for(int v_id = 0; v_id < samp_total_num_; v_id++){
        ...
        if(sensor_type_ == SensorType::CAMERA){
            ...
            list<Eigen::Vector3d> debug_pts, debug_pts0;
            for(int h = 0; h < FOV_h_num_; h++){
                double h_dir = vp(3) + double(h) / FOV_h_num_ * M_PI * 2 - M_PI;
                for(int v = 0; v < FOV_v_num_; v++){
                    bool valid_ray = false;
                    double v_dir = double(v+0.5) / FOV_v_num_ * (livox_ver_up_ - livox_ver_low_) + livox_ver_low_;
                    // tr1::unordered_map<int, double> l1;
                    // list<Eigen::Vector3d> ray1;
                    // list<pair<Eigen::Vector3d, double>> ray2;
                    Eigen::Vector3d dir;
                    ...
                    for(; l_gain < sensor_range_; l_gain += ray_samp_dist2_){
                        ...
                        Eigen::Vector3d pm = p + vp.block(0, 0, 3, 1);
                        if(valid_ray){
                            for(auto ddd : debug_pts0) debug_pts.emplace_back(ddd);
                            debug_pts.emplace_back(pm);
                        }
                        if(abs(pm.x()) > geh*node_scale_(0) / 2 || abs(pm.y()) > geh*node_scale_(1) / 2
                             || abs(pm.z()) > geh*node_scale_(2) / 2){
                            break;
