...
void EroiGrid::SampleVps(list<int> &idxs, bool reset_flag){
    ...
    Eigen::Vector3d vp_pos;
    list<Eigen::Vector3d> debug_pts_out, debug_pts_dead, debug_pts_infea;
    int vp_id;
    bool debug = false;
    ...
    for(auto &idx : idxs){
        ...
        for(int vp_id = 0; vp_id < samp_total_num_; vp_id++){
            ...
            if(EROI_[idx].local_vps_[vp_id] == 2) {
                // if(191710 == idx)  debug_pts_dead.emplace_back(vp_pos);
                // ROS_ERROR("out 1");
                // Debug(debug_pts);
                // getchar();
                continue;
            ...
            if(!LRM_->InsideLocalMap(vp_pos)) {
                // if(191710 == idx) debug_pts_out.emplace_back(vp_pos);
                continue;
            }
    ...
    // if(debug) Debug(debug_pts_out, debug_pts_dead, debug_pts_infea);
    // if(debug_pts_dead.size() != 0) getchar();
    // Debug(idxs);
}
...
void EroiGrid::UpdateFrontier(const vector<Eigen::Vector3f> &pts){
    int idx;
    list<int> idx_list;
    for(auto &p : pts){
        if(!InsideExpMap(p)) continue;
...
double EroiGrid::GetGain(const int &f_id, const int &vp_id){
    ...
    for(auto &d_l : gain_dirs_[vp_id]){
        ...
        for(auto &p : ray){
            ...
            state = BM_->GetVoxStateFrontier(p);
            // vn += 1;
...
bool EroiGrid::StrongCheckViewpoint(const int &f_id, const int &v_id, const bool &allow_unknown){
    ...
    for(auto &d_l : gain_dirs_[v_id]){
        ...
        if(g) gr++;
        if(gain > vp_thresh_ && gr > 5) return true;
    }
    // if(f_id == 191710) cout<<"gain: "<<gain<<"  gr:"<<gr<<endl;
    return false;
...
void EroiGrid::Debug(list<Eigen::Vector3d> &pts_out, list<Eigen::Vector3d> &pts_dead, list<Eigen::Vector3d> &pts_infea){
    visualization_msgs::Marker mk, mkr1, mkr2;
    mk.header.frame_id = "world";
    mk.header.stamp = ros::Time::now();
    mk.id = 0;
