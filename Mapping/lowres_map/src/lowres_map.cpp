...
void LowResMap::UpdateLocalBBX(const Eigen::Matrix4d rob_pose, const pair<Eigen::Vector3d, Eigen::Vector3d> bbx){
    mtx_.lock();
    ...
    Eigen::Vector3i node3i, upi, downi;
    ...
    local_up_idx_ = upi;
    local_low_idx_ = downi;
    // cout<<"upi:"<<upi.transpose()<<endl;
    // cout<<"downi:"<<downi.transpose()<<endl;
    // cout<<"bbx1:"<<bbx.first.transpose()<<endl;
    // cout<<"bbx2:"<<bbx.second.transpose()<<endl;
        ...
        for(node3i(1) = downi(1); node3i(1) <= upi(1); node3i(1)++){
            for(node3i(2) = downi(2); node3i(2) <= upi(2); node3i(2)++){
                ...
                if(node_status == 2){
                    ...
                    if(GetNode(pos) == NULL) SetEXPNode(pos);
                    SetNode(pos);
                    // debug_list.emplace_back(pos);
                    if(!IsFeasible(pos)){
                        cout<<pos.transpose()<<endl;
                        ROS_ERROR("error fea%d", GetBlockId(pos));
                    }
                }
            }
        }
