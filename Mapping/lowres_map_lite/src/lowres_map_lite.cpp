...
void LowResMap::ReloadNodes(const Eigen::Vector3d &r_pos, vector<pair<int, int>> &candidate_search_origins, 
    ...
    Eigen::Vector3d new_origin, half_blk;
    Robot_pos_ = r_pos;
    ...
    new_origin_idx = map_->local_origin_idx_;
...
void LowResMap::SetNodeStatesBf(vector<Eigen::Vector3d> &new_free_nodes){
    ...
    for(auto cn : map_->changed_pts_){
        ...
        if(LG_[bid]->local_grid_[nid] == Outnode_) {
            // ROS_WARN("out");
            continue;
        }
        ...
        for(int z = 0; z < block_size_(2); z++){
            for(int y = 0; y < block_size_(1); y++){
                for(int x = 0; x < block_size_(0); x++){
                    ...
                    Eigen::Vector3d pu, pd;
                    pd = pi.cast<double>().cwiseProduct(node_scale_) + origin_;
                    pu = pi.cast<double>().cwiseProduct(node_scale_) + origin_ + node_scale_;
                    if(!map_->InsideExpMap(pd) || !map_->InsideExpMap(pu)) continue;
                    ...
                    for(int xi = 0; xi < node_size_(0); xi++)
                        for(int yi = 0; yi < node_size_(1); yi++)
                            for(int zi = 0; zi < node_size_(2); zi++){
                                ...
                                else if(cstatus == VoxelState::occupied) occ++;
                                else u++;
                                // if(cstatus != VoxelState::unknown && cstatus != VoxelState::out){
                                //     Eigen::Vector3f pf(p(0), p(1), p(2));
                                //     uint64_t idx = map_->PostoId(pf);
                                //     if(map_->changed_pts_.find(idx) == map_->changed_pts_.end()){
                                //         ROS_ERROR("not find key");
                                //         // error_flag = true;
                                //     }
                                // }
                                // else{
                    ...
                    //     cout<<"f:"<<int(f)<<"  u:"<<int(u)<<" occ:"<<int(occ)<<" out:"<<int(out)<<endl;
                    ...
                    if(f != g->local_grid_[id]->free_num_ || u != g->local_grid_[id]->unknown_num_){
                        ...
                        cout<<"f:"<<int(f)<<"  u:"<<int(u)<<" occ:"<<int(occ)<<endl;
                        cout<<"g->f:"<<int(g->local_grid_[id]->free_num_)<<"  g->u:"<<int(g->local_grid_[id]->unknown_num_)<<endl;
                        ...
                        for(int xi = 0; xi < node_size_(0); xi++)
                            ...
                            p = pi.cast<double>().cwiseProduct(node_scale_) + Eigen::Vector3d(0.5+xi, 0.5+yi, 0.5+zi) * resolution_ + origin_;
                            auto cstatus = map_->GetVoxState(p);
                            if(cstatus == VoxelState::free || cstatus == VoxelState::occupied) {
                                Eigen::Vector3f pf(p(0), p(1), p(2));
                                uint64_t idx = map_->PostoId(pf);
                                if(map_->changed_pts_.find(idx) != map_->changed_pts_.end()){
                                    cout<<"s:"<<int(map_->changed_pts_.find(idx)->second.first)<<"   ss:"<<int(map_->changed_pts_.find(idx)->second.second)<<endl;
                                }
                            }

                        }

                        cout<<"change pts:"<<map_->changed_pts_.size()<<endl;
                        cout<<"g_id:"<<g_id<<endl;
                        cout<<"bid:"<<map_->GetBlockId(p)<<endl;
                        cout<<"local_map_lowbd_:"<<local_map_lowbd_.transpose()<<endl;
                        cout<<"local_map_upbd_:"<<local_map_upbd_.transpose()<<endl;
