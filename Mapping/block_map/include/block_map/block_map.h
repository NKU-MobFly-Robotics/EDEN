...
public:
    ...
    /**
     * @brief update map through shadow cast
     * 
     * @param depth 
     */
...
private:
    ...
    Eigen::Vector3d local_origin_, local_upbd_, local_scale_;
    Eigen::Vector3i local_block_num_, local_origin_idx_, local_up_idx_;
...
inline void BlockMap::Flags2Vox(vector<uint8_t> &flags, Eigen::Vector3d &up, Eigen::Vector3d &down,
    ...
    Eigen::Vector3i v_n;
    int vox_num = 1;
    int flag_num = flags.size() * 4;    string voxel_path_;
...
inline uint8_t BlockMap::EncodeVoxel(const uint8_t *c){
    uint8_t d = 0;
    for(int i = 0; i < 5; i++){
        d = d * 3 + c[i];
    }
...
inline void BlockMap::DecodeVoxel(const uint8_t &d, uint8_t *c){
    uint8_t dd = d;
    for(int i = 0; i < 5; i++){
        c[4 - i] = d % 3;
        dd = dd / 3;
...
inline bool BlockMap::GetVox(int &block_id, int &vox_id, const Vector3d &pos){
    ...
    if(block_id != -1){
        if(block_id>=GBS_.size()){
            ...
            cout<<posid(2)*block_num_(0)*block_num_(1) + posid(1)*block_num_(0) + posid(0)<<endl;
            cout<<"size:"<<GBS_.size()<<endl;
            ROS_ERROR("error");
            getchar();
        }
...
inline int BlockMap::GetLocalBlockId(const Eigen::Vector3i &pos){
...
inline Eigen::Vector3i BlockMap::Pos2Id3Inside(const Eigen::Vector3d &p){
    Eigen::Vector3d P;
    P(0) = min(p(0), local_upbd_(0));
    P(1) = min(p(1), local_upbd_(1));
    P(2) = min(p(2), local_upbd_(2));
    P(0) = max(P(0), local_origin_(0));
    P(1) = max(P(1), local_origin_(1));
    P(2) = max(P(2), local_origin_(2));
    return PostoId3(P);
}



inline bool BlockMap::GetXLineIntersec(const Eigen::Matrix3d &R, const Eigen::Vector3d &trans, const double &y, const double &z, double &xd, double &xu){
    double py, pz;
    py = y - trans(1);
    pz = z - trans(2);
...
inline bool BlockMap::Cast2Img(const Eigen::Matrix3d &R, const Eigen::Vector3d &trans, const Eigen::Vector3d &p, int &x, int &y, double &depth){
