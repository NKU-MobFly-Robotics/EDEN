...
void BlockMap::ResetLocalMap(const Eigen::Vector3d &p){
    Eigen::Vector3d new_origin;
    Eigen::Vector3i new_origin_idx, new_up_idx, it, diff;
    new_origin = p - local_scale_ * 0.5;
    for(int dim = 0; dim < 3; dim++) new_origin(dim) = min(max(new_origin(dim), map_lowbd_(dim)), map_upbd_(dim));
    ...
    new_up_idx = new_origin_idx + local_block_num_;
    ...
    for(int x = 0; x < local_block_num_(0); x++){
        for(int y = 0; y < local_block_num_(1); y++){
            for(int z = 0; z < local_block_num_(2); z++){
                ...
                if(isc) out_idxs.emplace_back(it);
                else {
                    ...
                    lid = GetBlockId(it);
                    if(lid < 0) { //debug
                        ROS_ERROR("ResetLocalMap() impossible GetBlockId0");
                        ros::shutdown();
                        continue;
                    }
    ...
    for(auto &b : out_idxs){
        ...
        if(lid < 0) { //debug
            ROS_ERROR("ResetLocalMap() impossible GetLocalBlockId1");
            ros::shutdown();
            continue;
        }
        threads_w.emplace_back(&BlockMap::SaveData, this, std::ref(id), std::ref(GBS_local_[lid]));
    }
    ...
    for(auto &bid : change_idxs){
        ...
        change_list.emplace_back(lid, GBS_local_[bid.first]);
    }
    ...
    for(auto &bid: new_idxs){
        ...
        threads_r.emplace_back(&BlockMap::ReadData, this, std::ref(bid.first), std::ref(GBS_local_[lid]));
    }
...
void BlockMap::CastInsertImg(const sensor_msgs::ImageConstPtr &depth){
    if(!have_cam_param_) return;
    vector<int> block_ids, vox_ids;
    vector<int>::iterator block_it, vox_it;
    ...
    if(InsideMap(cam3i)){
        ...
        for(int v = 0; v < v_max_; v += depth_step_){
            ...
            for(int u = 0; u < u_max_; u += depth_step_, row_ptr += depth_step_){
                ...
                if(InsideMap(end_point)){
                    // vox_id = GetVoxId(end_point);
                    GetVox(block_id, vox_id, end_point);
                    if(occ && !(GBS_[block_id]->flags_[vox_id] & 2) /*!(VX_flags_[vox_id] & 2)*/){
                        GBS_[block_id]->flags_[vox_id] = 3;
        ...
        for(it(2) = downi(2); it(2) <= upi(2); it(2)++, p_it(2) += resolution_){
            ...
            for(it(1) = downi(1); it(1) <= upi(1); it(1)++, p_it(1) += resolution_){
                ...
                for(it(0) = xdi; it(0) <= xui; it(0)++, p_it(0) += resolution_){
                    // pi(0) = (it(0) + 0.5) * resolution_;
                    // pi(1) = (it(1) + 0.5) * resolution_;
                    // pi(2) = (it(2) + 0.5) * resolution_;
                    // pi += origin_;
                    ...
                    // vox_id = it(0) + (it(1) + it(2) * voxel_num_(1)) * voxel_num_(0);
                    GetVox(block_id, vox_id, p_it);
                    // if(block_id < 0 || block_id >= GBS_.size() || vox_id < 0 || vox_id >= GBS_[block_id]->flags_.size()){
                    //     ROS_ERROR("111");
                    //     cout<<"block_id:"<<block_id<<endl;
                    //     cout<<"vox_id:"<<vox_id<<endl;
                    //     cout<<"p_it:"<<p_it.transpose()<<endl;
                    //     cout<<"xu:"<<xui * resolution_ + origin_(0) + 0.5 * resolution_<<endl;
                    //     cout<<"xd:"<<xdi * resolution_ + origin_(0) + 0.5 * resolution_<<endl;
                    //     cout<<"GBS_:"<<GBS_.size()<<endl;
                    //     cout<<"flags_:"<<GBS_[block_id]->flags_.size()<<endl;
                    //     getchar();
                    // }
                    if(!(GBS_[block_id]->flags_[vox_id] & 1)){
                        GBS_[block_id]->flags_[vox_id] |= 1;
