...
void MultiDtgPlus::FindFastExpTarget( const vector<h_ptr> &route, const Eigen::Vector4d &ps, 
    ...
    for(auto &hfe : candidate_hfe){
        ...
        if(LRM_->InsideLocalMap(eroi.center_)){
                ...
                if(eroi.local_vps_[i] == 1){
                    ...
                    if(LRM_->Connected(vp_pos)){
                        ...
                        if(gain1 > best_gain){
                            ...
                            first_branch = false;
                            find_first = true;
                            debug_int = 1; // go to single pt, vp not in local
                            vp_best1 = vp.head(3);
                        }
                    }
                }
            ...
            if(!local_fastexp_candidate_vps.empty()){
                ...
                for(auto &vp1 : local_fastexp_candidate_vps){
                    ...
                    if(gain1 > best_gain){
                        ...
                        first_branch = false;
                        find_first = true;
                        debug_int = 2; // go to single pt, in local
                        vp_best1 = vp_pos;
                    }
            ...
            if(gain1 > best_gain){
                ...
                find_first = true;
                debug_int = 4; // go to single pt, eroi out local
                first_branch = false;
                vp_best1 = vp_pos;
            }
        }
    ...
    // Debug(debug_vps);
    // ROS_WARN("FindFastExpTarget1");
    // cout<<"gain best:"<<best_gain<<endl;


    if(!find_first) {
        cout<<"hfe num:"<<route.front()->hf_edges_.size()<<endl;
        cout<<"best_gain:"<<best_gain<<endl;
        cout<<"gain1:"<<gain1<<endl;
        cout<<"gain2:"<<gain2<<endl;
        for(auto hfe : route.front()->hf_edges_){
            cout<<"hfe f state:"<<int(EROI_->EROI_[hfe->tail_n_->fid_].f_state_)<<endl;
            cout<<"in local:"<<LRM_->InsideLocalMap(EROI_->EROI_[hfe->tail_n_->fid_].center_)<<endl;
        }
        
        StopageDebug("FindFastExpTarget no first target");
    }
    /* clear the first vp */
    for(auto i = connected_vps.begin(); i != connected_vps.end(); i++){
        if(i->first == best_tar1.first && i->second == best_tar1.second){
            ROS_WARN("FindFastExpTarget1.8");
    ...
    for(auto i = not_connected_vps.begin(); i != not_connected_vps.end(); i++){
        if((*i)->tail_n_->fid_ == best_tar1.first && (*i)->tail_n_->vid_ == best_tar1.second){
            ROS_WARN("FindFastExpTarget1.9");
            f_hfe = *i;
            not_connected_vps.erase(i);
            break;
        }
    }
    ROS_WARN("FindFastExpTarget2");
    cout<<"VP1:"<<int(best_tar1.first)<<"   "<<int(best_tar1.second)<<endl;
    cout<<"debug_int:"<<int(debug_int)<<endl;
    cout<<"gc:"<<gc<<endl;
    cout<<"continuous_m:"<<continuous_m<<endl;
    cout<<"VP1:"<<(EROI_->EROI_[best_tar1.first].center_ + EROI_->vps_[best_tar1.second].head(3)).transpose()<<" "<<EROI_->vps_[best_tar1.second](3)<<endl;
    ...
    if(LRM_->Connected(vp_best1)){
        ...
        ROS_WARN("FindFastExpTarget2.2");

        path_stem = path_norm;
        y_stem = EROI_->YawDiff(EROI_->vps_[best_tar1.second](3), ps(3)) + ps(3);
        y_norm = y_stem;       
        if(first_branch) {
            ROS_WARN("FindFastExpTarget2.21");
            cout<<"VP2:"<<(EROI_->EROI_[best_tar2.first].center_ + EROI_->vps_[best_tar2.second].head(3)).transpose()<<endl;
        ...
        if(!not_connected_vps.empty() || !connected_vps.empty()){
            ...
            for(auto &hfe : not_connected_vps){
                gain_sec = exp(-lambda_ * (h_dist + hfe->length_) / v_max_); // yaw does not matter
            ...
            if(LRM_->Connected(vp_pos)){
                ...
                ROS_WARN("FindFastExpTarget2.541");
                LRM_->RetrieveHPath(si, path_sub, l, true);
                y_sub = y_stem;
            }
            else{
                ROS_WARN("FindFastExpTarget2.542");
            ...
            if(!first_branch){
                ROS_WARN("FindFastExpTarget2.543");
                tar_main = best_tar2;
                path_main = path_sub;
                y_main = y_sub;
            }
            ROS_WARN("FindFastExpTarget2.55");

        }
        else if(route.size() > 1){
            ROS_WARN("FindFastExpTarget2.56");
            auto h2 = route[1];
            if(!Astar(vp_best1, p_hs, g0_hs, h2, h2_paths, path_2_best_h, h_dist)){ // dont comment out
                StopageDebug("FindFastExpTarget Astar failed");
            }
            ROS_WARN("FindFastExpTarget2.57");
            ...
            ROS_WARN("FindFastExpTarget2.58");

            vp = EROI_->vps_[sbest_tar1.second];
            vp.head(3) += EROI_->EROI_[sbest_tar1.first].center_;
            vp_pos = vp.head(3);
            tar_sub = sbest_tar1;
            ROS_WARN("FindFastExpTarget2.581");

            if(LRM_->Connected(vp_pos)){
                ROS_WARN("FindFastExpTarget2.582");
                double ls;
                Eigen::Vector3i si = LRM_->GlobalPos2GlobalId3(vp_pos);
                LRM_->RetrieveHPath(si, path_sub, l, true);
                y_sub = y_stem;
                ...
                ROS_WARN("FindFastExpTarget2.583");
                path_sub = path_2_best_h;
                path_sub.pop_back();
                ROS_WARN("FindFastExpTarget2.584");
                ...
                path_sub.insert(path_sub.end(), s_hfe->path_.begin(), s_hfe->path_.end());
                ROS_WARN("FindFastExpTarget2.585");

                LRM_->GetPathInLocal(vp(3), path_sub, y_sub, path_sub);
                ROS_WARN("FindFastExpTarget2.586");
                y_sub = y_stem;
            }

            if(!first_branch){
                ROS_WARN("FindFastExpTarget2.587");

                tar_main = best_tar2;
                path_main = path_sub;
                y_main = y_sub;
            }
            ROS_WARN("FindFastExpTarget2.59");

        }
        ROS_WARN("FindFastExpTarget2.6");

    }
    else{
        ROS_WARN("FindFastExpTarget2.7");

        tar_norm = best_tar1;
        plan_res = 3;
        path_norm = path1;
        if(f_hfe == NULL){
            cout<<"best_tar1:"<<best_tar1.first<<"  "<<int(best_tar1.second)<<endl;
            cout<<"best_tar1:"<<EROI_->EROI_[best_tar1.first].center_<<"  "<<EROI_->vps_[best_tar1.second]<<endl;
            cout<<"vp_best1:"<<vp_best1.transpose()<<endl;
            cout<<"first_branch:"<<first_branch<<endl;
            StopageDebug("FindFastExpTarget f_hfe == NULL");
        }
        ROS_WARN("FindFastExpTarget2.8");
        if(tar_norm.second >= EROI_->vps_.size()) {
            StopageDebug("FindFastExpTarget, tar_norm.second >= EROI_->vps_.size()");
        }
        if(path_norm.size() == 0){
            StopageDebug("path_norm.size() == 0");
        }
        cout<<"path_norm:"<<path_norm.size()<<"  f_hfe->path_:"<<f_hfe->path_.size()<<endl;
        ...
        ROS_WARN("FindFastExpTarget2.9");

    }
    ROS_WARN("FindFastExpTarget3");
    cout<<"stem eroi id:"<<tar_stem.first<<"   stem vpid:"<<int(tar_stem.second)<<endl;
    cout<<"sub eroi id:"<<tar_sub.first<<"   sub vpid:"<<int(tar_sub.second)<<endl;
    cout<<"main eroi id:"<<tar_main.first<<"   main vpid:"<<int(tar_main.second)<<endl;
    cout<<"norm eroi id:"<<tar_norm.first<<"   norm vpid:"<<int(tar_norm.second)<<endl;
